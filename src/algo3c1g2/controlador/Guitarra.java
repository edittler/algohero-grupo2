...
public class Guitarra implements ObjetoVivo {
	...
	public Guitarra(Cancion unaCancion, ControladorJuego controlador) {
		...
		/* Se crean las 6 cuerdas, cada una crea una cuerda con
		 *  una ubicacion (en el mástil) con 5 circulitos habilitables
		 */
		...
		// Esto deberia depender del tempo me parece (Ezequiel)
		Circulito.pixelesPorCiclo = 2;

		// esto no es necesario lo agrege para q termine el juego despues hay q
		// cambiarlo TODO
	...
	// TODO Hay """""malos olores""""" en estos métodos si encuentran algo
	// avisenme asi lo corregimos!
	private void addCirculito(ElementoDeCompas elemento) {
		...
		if (elemento != null) {
			...
			if (elemento.isNota()) {
				...
				/* Habilita un Circulito en la Cuerda correspondiente
				 *  a la Nota y obtiene una referencia al Circulito habilitado
				 */
		...
		if (this.cancion.verificarTeclas(unaCombinacionTeclas, this.getInstanteDeCancion(), Guitarra.PRESICION_SIMULACION)) {
			...
			while (itCuerdas.hasNext() && !conto) {
				...
				while (itCir.hasNext() && !conto) {
					...
					if (this.tablaPuntos.estaDentroDelRango(unCirculito.getY())) { // TODO
																					// REFACTORIZAME!!!
	...
	private void agregarCirculosAControlador() {
		...
		while (itCuer.hasNext()) {
			...
			Iterator<ImageCirculito> itVistas = unaCuerda.iteratorVista();
